...
[ECS Fargate] Spring Boot 3.x                                                                                                                                                 
   ...
   [JWT Filter] [OAuth2] │
      │                  │                                                                                                                                                    
      ├── Google Maps Places API  ← 주변 장소 검색          
      ├── Naver Blog Search API   ← 리뷰 수집                                                                                                                                 
      ├── OpenAI API              ← 리뷰 분석 / 태그                                                                                                                          
      ├── Open-Meteo              ← 날씨 데이터                                                                                                                               
      ├── Expo Push API           ← 앱 푸시 알림                                                                                                                              
      ├── Python FastAPI (AI 서버) ← 동선 최적화 SSE                                                                                                                          
      ├── PostgreSQL (Supabase)   ← 장소·여행·사용자                                                                                                                          
      └── Redis (ElastiCache)     ← 토큰·캐시·분산락·혼잡도                                                                                                                   
